...
all: $(OUTPUT_DIRECTORY) $(TARGET).bin
...
$(TARGET).out: $(SRC_FILES)
	...
	"/Library/Developer/CommandLineTools/usr/bin/make" -s --no-print-directory -f "/Users/nick/projects/iot//nRF5_SDK_17.1.0_ddde560//components/toolchain/gcc/dump.mk" VARIABLE=CONTENT_TO_DUMP > $(OUTPUT_DIRECTORY)/$(TARGET).in
	"/Applications/ARM/bin/arm-none-eabi-gcc" -O3 -g3 -mthumb -mabi=aapcs -L /Users/nick/projects/iot/nRF5_SDK_17.1.0_ddde560/modules/nrfx/mdk -L /Users/nick/projects/iot/nRF5_SDK_17.1.0_ddde560/modules/nrfx/mdk/nrf_common.ld -T $(PROJ_DIR)/wfe_gcc_nrf52.ld -mcpu=cortex-m4 -mfloat-abi=hard -mfpu=fpv4-sp-d16 -Wl,--gc-sections --specs=nano.specs $(OUTPUT_DIRECTORY)/$(TARGET).in -Wl,-Map=$(OUTPUT_DIRECTORY)/$(TARGET).map -o $(OUTPUT_DIRECTORY)/$(TARGET).out
%.S.o %.s.o.o:
